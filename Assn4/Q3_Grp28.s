...
# s1 -> n (element to be searched for)
# s2 -> return value from search function
...
sortArray:
    ...
    continueLoop:
        ...
        jal     SWAP                        # swap arr[r] and arr[l]

        j		sortLoop				    # jump to sortLoop

    sorted:
        lw          $ra, 0($sp)             # restoring the stored values
        lw          $s1, 4($sp)
        lw          $s0, 8($sp)
        lw          $s2, 12($sp)
        lw          $s3, 16($sp)
...
recurse_search:
    ...
    beq             $t0, $zero, recurse_search_continue  # if a1<=a2 then go to recurse, else return -1
    li              $v0, -1                 # reuturn -1
    jr              $ra                     # jump to ra
